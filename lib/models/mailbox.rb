class Mailbox
  ...
  def emails(key_or_opts = :all, opts={})
    ...
    if !opts.empty?
      ...
      search.concat ['SINCE', to_imap_date(opts[:after])] if opts[:after]
      search.concat ['BEFORE', to_imap_date(opts[:before])] if opts[:before]
      search.concat ['ON', to_imap_date(opts[:on])] if opts[:on]
  ...
  def to_imap_date(str)
    Date.parse(str.to_s).strftime("%d-%B-%Y")
  end
end
